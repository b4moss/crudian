...
before(async () => {
  tempDir = mkdtempSync(join(tmpdir(), "crudian-libsql-pk-"))
  const dbPath = join(tempDir, "test.db")
  client = createClient({ url: `file:${dbPath}` })
  await client.execute(ID_SCHEMA)
  await client.execute(ITEM_ID_SCHEMA)
})
...
describe("libsql.createCrudPk", () => {
  test("正常系: カスタム PK で create / search paging / upsert / duplicate", async () => {
    await client.execute("DELETE FROM items_pk")
