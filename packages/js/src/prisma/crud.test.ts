...
async function resetTable(c: PrismaClient) {
  await c.$executeRawUnsafe("DELETE FROM items")
  await c.$executeRawUnsafe("DELETE FROM sqlite_sequence WHERE name = 'items'")
}
...
before(async () => {
  tempDir = mkdtempSync(join(tmpdir(), "crudian-prisma-"))
  const dbPath = join(tempDir, "test.db")
  process.env.DATABASE_URL = `file:${dbPath}`
  client = new PrismaClient()
  clients.push(client)
  await client.$executeRawUnsafe(SCHEMA)
})
...
describe("prisma.searchList", () => {
  test("正常系: デフォルト offset と list 別名", async () => {
    const crud = await crudFixture()
    for (let i = 0; i < 5; i++) {
    ...
    assert.equal("offset" in page1 && page1.offset, 0)
    assert.equal("limit" in page1 && page1.limit, 2)
    assert.equal("nextCursor" in page1, false)
    const page2 = await crud.search("items", { limit: 2, offset: 2 })
    assert.deepEqual(
  ...
  test("正常系: cursor 明示", async () => {
    ...
    assert.equal("offset" in page1, false)
...
describe("prisma.extendedWrites", () => {
...
describe("prisma.adapterExports", () => {
