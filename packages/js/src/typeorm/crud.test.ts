...
async function resetTable(ds: DataSource) {
  await ds.query("DELETE FROM items")
  try {
    await ds.query("DELETE FROM sqlite_sequence WHERE name = 'items'")
  } catch {
...
async function crudFixture() {
  await resetTable(dataSource)
  return createCrud(dataSource)
}

before(async () => {
  tempDir = mkdtempSync(join(tmpdir(), "crudian-typeorm-"))
...
describe("typeorm.createCrud", () => {
  test("正常系: 注入 DataSource と同一参照", async () => {
    const crud = await crudFixture()
    assert.equal(crud.db, dataSource)
  })
  ...
  test("正常系: options.type から dialect を推定する", async () => {
    const crud = createCrud(dataSource)
    const row = await crud.create("items", { name: "dialect", score: 1 })
    assert.equal(row.name, "dialect")
  })
})
...
describe("typeorm.searchList", () => {
  test("正常系: offset デフォルトと cursor 明示 / list 別名", async () => {
    const crud = await crudFixture()
    for (let i = 0; i < 5; i++) {
      await crud.create("items", { name: `n${i}`, score: i })
    }

    const offset1 = await crud.search("items", { limit: 2 })
    ...
    const offset2 = await crud.search("items", { limit: 2, offset: 2 })
    assert.deepEqual(
    ...
    const page2 = await crud.search("items", {
      paging: "cursor",
      limit: 2,
  ...
  test("異常系: limit / in 空 / 相反入力 / 不正 offset", async () => {
    ...
    await assert.rejects(() => crud.search("items", { cursor: 1 }), CrudianError)
    await assert.rejects(
...
describe("typeorm.extendedWrites", () => {
...
describe("typeorm.adapterExports", () => {
