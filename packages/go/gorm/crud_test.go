...
func mustCreateCrud(t *testing.T, db *gorm.DB) *gormcrud.Crud {
	t.Helper()
	crud, err := gormcrud.CreateCrud(db)
	if err != nil {
...
func TestCreateCrud(t *testing.T) {
	...
	if crud.DB != db {
		t.Fatal("expected same *gorm.DB")
	}
	if _, err := gormcrud.CreateCrud(nil); err == nil {
...
func TestSearchListCount(t *testing.T) {
	...
	_, err = crud.Create(ctx, "items", crudian.Row{"name": "n", "score": 0, "note": nil})
	if err != nil {
		// SQLite may need explicit NULL — omit note column instead
		_, err = crud.Create(ctx, "items", crudian.Row{"name": "n", "score": 0})
		if err != nil {
			t.Fatal(err)
		}
		if err := crud.DB.Exec(`UPDATE items SET note = NULL WHERE name = ?`, "n").Error; err != nil {
			t.Fatal(err)
		}
	}
